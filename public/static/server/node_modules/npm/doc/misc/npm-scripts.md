...
* `"start": "node connectServer.js"`:

  If there is a `connectServer.js` file in the root of your package, then npm
  will default the `start` command to `node connectServer.js`.
...
and the connectServer.js is this:
