...
<div class="x-body" id="vueData">
    <fieldset class="layui-elem-field">
        <legend>员工管理</legend>
        <div class="layui-field-box" id="allemp">
            <table class="layui-table" lay-even>
                <tbody>

                <!--角色管理操作-->
                <tr v-for="empInfo in allemp" v-if="empInfo.roleid==1">
                    <td>{{empInfo.empid}}</td><td>{{empInfo.empname}}</td><td>{{empInfo.rolename}}</td><td>{{empInfo.empstate}}</td><td>{{empInfo.loginstate}}</td><td>--</td><td>--</td>
                </tr>
                <tr v-for="empInfo in allemp" v-if="empInfo.roleid!=1">
                    <td>{{empInfo.empid}}</td><td>{{empInfo.empname}}</td><td>{{empInfo.rolename}}</td><td>{{empInfo.empstate}}</td><td>{{empInfo.loginstate}}</td>
                    <td>
                        <a :rid="empInfo.empid" data-toggle="modal" data-target="#modalEdit" onclick="empEdit(this)" href="#">权限/修改</a>
                    </td>
                    <td>
                        <a :rid="empInfo.empid" onclick="empDel(this)" href="#">删除</a>
                    </td>
                </tr>
                </tbody>
            </table>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="layui-form-item">
                    ...
                    <div class="content_wrap layui-input-inline">
                        <div class="zTreeDemoBackground left">
                            <ul id="empEdit" class="ztree"  style="background-color: whitesmoke;width: 192px;height: 100%;border-color: #e6e6e6"></ul>
                        </div>
                    </div>
...
<script>
    ...
    $('#showAdd').click(function(){
        $.ajax({
            url:"<?php echo url('admin/backmanager/getEmpMenu'); ?>",
            data:'',
            type:'post',
            dataType:'json',
            success:function(res){
    ...
    $("#nameAdd").blur(function(){
        ...
        $newEmpName=$newEmpName.replace(/(^\s*)|(\s*$)/g, "");
        //$.trim($newEmpName)
        ...
        if($newEmpName==''){
            ...
            //判断重复
            $.ajax({
                url:"<?php echo url('admin/backmanager/checkEmp'); ?>",
                data:'empname='+$newEmpName,
                type:'post',
                dataType:'json',
                success:function(res){
                    if(res.code==20007){
    ...
    $("#empAdd").click(function (){
        ...
        if($AjaxCheck==1){
            $.ajax({
                url:"<?php echo url('admin/backmanager/EmpAdd'); ?>",
                data:{
                    'empname':nameAdd,
                    'empintro':introAdd,
                    'empmenu':newmenuid
    ...
    //角色权限修改
    function empEdit(obj){
        $rid = $(obj).attr('rid');//获取角色ID
        $.ajax({
            url:"<?php echo url('admin/backmanager/empEditData'); ?>" ,
            data:'rid='+$rid,
            type:'post',
            dataType:'json',
            success:function(res){
                ...
                if(res!=''){
                    ...
                    //给对应得文本框绑定内容
                    $("#nameEdit").val(res.data[1][0]['empname']);
                    $("#introEdit").val(res.data[1][0]['headimg']);

                    //给当前的元素增加属性'',属性值为''
                    $("#nameEdit").attr("empid",res.data[1][0]['empid']);
    ...
    $("#saveEdit").click(function(){
        //获取对应ztree(id)节点的选中数据
        var empEdit = $.fn.zTree.getZTreeObj("empEdit");
        //获取选中(true)的所有属性
        var newmenu = empEdit.getCheckedNodes(true);
        var newmenuid = [];
        for(var $i = 0;$i<newmenu.length;$i++){
            newmenuid.push({
                menuid:newmenu[$i]['id']
        ...
        //获取当前被修改角色id
        var editId = $("#nameEdit").attr('empId');
        ...
        //删除数据+//插入数据
        $.ajax({
            url:"<?php echo url('admin/backmanager/empEdit'); ?>",
            //data数据角色id,角色名,角色介绍
            //复数数据使用json
            data:{'empid':editId,'empname':editName,'headimg':editDetails,'menuid':newmenuid},
            type:'post',
            dataType:'json',
            success:function(res) {
                alert(res.msg);
